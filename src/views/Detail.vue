<template>
  <div class="m-wiki-wrapper">
    <Search/>
    <KnowledgeSingle v-if="wiki_post && wiki_post.source" :knowledge="wiki_post.source" :fav-enable="true"
                     :deep="true"/>
    ...
    watch: {
      'id': {
        ...
        handler() {
          // 获取最新攻略
          if (this.id) {
            WikiPost.newest('knowledge', this.id).then(
                (res) => {
                  res = res.data;
                  if (res.code === 200) {
                    this.wiki_post = res.data;
                    if (this.wiki_post.source) this.wiki_post.source.post = this.wiki_post.post;
                  }
                }
            );
          }

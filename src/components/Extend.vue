    ...
    <div class="m-side-rank">
      ...
      <ul class="m-rank-list" v-if="data.length">
        <li v-for="(item, i) in data" :key="i">
          <router-link class="u-link" :to="{name: 'view', params: {source_id: item.id}}">
            <span class="u-order" :class="highlight(i)" v-text="i + 1"></span>
    ...
    mounted: function () {
      getStatRank('knowledge').then((res) => {
        ...
        get_list({
          ...
          res = res.data;
          if (res.code === 200) this.data = res.data.data;
        });
      });
    },
    components: {},
